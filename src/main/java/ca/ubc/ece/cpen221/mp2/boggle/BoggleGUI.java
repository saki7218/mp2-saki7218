 ...
 *  % java BoggleGUI
 *
 *  % java -Xmx300m BoggleGUI 3 7
 *
 ******************************************************************************/

import ca.ubc.ece.cpen221.utils.In;
import ca.ubc.ece.cpen221.utils.StdRandom;
...
public class BoggleGUI extends JFrame {
    ...
    // if making adjustments to levels, endGame (~line 400) contains
    // hard-coded elements
    ...
    private TreeSet<String> shakespeareDictionary;
    private TreeSet<String> nurseryDictionary;
    private TreeSet<String> commonDictionary;
    private TreeSet<String> algs4Dictionary;
    // GUI elements
    private JMenuBar menuBar;
    private JMenu gameMenu;
    private JRadioButtonMenuItem[] difficultySelection;
    private BogglePlayer solver;
     ...
     */
    public BoggleGUI(int rows, int cols) {
        ...
        In in1 = new In(new File("datasets/dictionary-shakespeare.txt"));
        shakespeareDictionary = new TreeSet<String>();
        for (String s : in1.readAllStrings()) {
            shakespeareDictionary.add(s);
        }

        // all words in common nursery rhymes
        In in2 = new In(new File("datasets/dictionary-nursery.txt"));
        nurseryDictionary = new TreeSet<String>();
        ...
        solver = new BogglePlayer(dictionary);
            ...
            public void run() {
                ...
                if (rows <= 0 || cols <= 0) {
                    throw new IllegalArgumentException("Rows and columns must be positive"
                            + "\nUsage: java BoggleGUI "
                            + "\nor:    java BoggleGUI [rows]"
                            + "\nor:    java BoggleGUI [rows] [cols]");
                }
                new BoggleGUI(rows, cols).setVisible(true);
            }
        });
    }
